...
This Contao bundle provides reusable, presentation-ready content elements. It intentionally does not provide structural layout controllers or shared frontend assets.

## Repository-specific role

This repository is responsible for actual Contao content elements.

Content elements should render concrete frontend content.

Do not move layout-controller logic into content elements unless explicitly required.

Keep element templates, backend labels, DCA fields and palettes consistent.

Registered content elements live in the `vtxm` content element category and are registered through `src/Resources/contao/config/config.php`.

Keep templates under `src/Resources/contao/templates/` aligned with the documented root classes and hooks.

This bundle is theme-agnostic. Do not add frontend CSS or JavaScript here unless explicitly requested.

MultiColumnWizard fields are used by structured elements such as tabs, accordion, timeline, factsbox, link list and slider. Preserve that editing model unless a task explicitly changes it.
...
- This bundle currently should not include frontend JavaScript; slider behavior belongs in `frontend-assets` or project assets.
...
- Keep `cssID` support intact where elements already support it.
...
- This bundle should expose stable HTML hooks; shared styling belongs in `frontend-assets` or project CSS.
